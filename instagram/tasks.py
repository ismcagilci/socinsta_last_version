...
def user_feed(user_pk, assistant_id):
    ...
    if len(user_posts) == 0:
        ...
        if len(new_action_count) != 0:
            ...
            new_action = action_name(post_pk=post_pk, assistant=assistant, instagram_account=instagram_account,
                                     ig_user=ig_user, source=source, source_type=source_type, relationship=relationship,
                                     status=9)
            new_action.save()
        ...
        for i in action:
            if assistant_type == 3:
                ...
                try:
                    ...
                    i.update_time = datetime.now(timezone.utc)
                    i.save()
                    api_error = Api_Error(instagram_account=assistant.instagram_account, error_action_type=6, api_error_mean=str(e),
                                          error_source="prepare_filtered_users")
                    api_error.save()
                    break
    ...
    if assistant_type == 0:
        ...
        try:
            ...
            if str(e) == "Expecting value: line 1 column 1 (char 0)":
                ...
                follow_action.update_time = datetime.now(timezone.utc)
                follow_action.save()
            api_error = Api_Error(instagram_account=assistant.instagram_account, error_action_type=7, api_error_mean=str(e),
                                  error_source="executioner_follow")
            api_error.save()
    elif assistant_type == 1:
        ...
        try:
            ...
            if str(e) == "Expecting value: line 1 column 1 (char 0)":
                ...
                like_action.update_time = datetime.now(timezone.utc)
                like_action.save()
            api_error = Api_Error(instagram_account=assistant.instagram_account, error_action_type=8, api_error_mean=str(e),
                                  error_source="executioner_like")
            api_error.save()
    elif assistant_type == 2:
        ...
        try:
            ...
            if str(e) == "Expecting value: line 1 column 1 (char 0)":
                ...
                comment_action.update_time = datetime.now(timezone.utc)
                comment_action.save()
            api_error = Api_Error(instagram_account=assistant.instagram_account, error_action_type=9, api_error_mean=str(e),
                                  error_source="executioner_comment")
            api_error.save()
    elif assistant_type == 3:
        ...
        try:
            ...
            if str(e) == "Expecting value: line 1 column 1 (char 0)":
                ...
                unfollow_action.update_time = datetime.now(timezone.utc)
                unfollow_action.save()
            api_error = Api_Error(instagram_account=assistant.instagram_account, error_action_type=10, api_error_mean=str(e),
                                  error_source="executioner_unfollow")
            api_error.save()
...
def create_white_list_users(username, assistant_id, white_list=0):
    ...
    if len(white_list_assistant) == 0:
        white_list_assistant = White_List_Assistant(instagram_account=active_ig_account, status=9,
    ...
    if white_list == 0:
        ...
        if user_followings:
            for i in user_followings:
                white_list_user = White_List_Users.objects.filter(instagram_account=active_ig_account,
                                                                  username=i.get('username'))
                if len(white_list_user) == 0:
                    white_list_user = White_List_Users(username=i.get('username'), instagram_account=active_ig_account,
                                                       assistant=white_list_assistant)
        ...
        for i in white_list:
            white_list_user = White_List_Users.objects.filter(instagram_account=active_ig_account,
                                                              username=i.get('username'))
            if len(white_list_user) == 0:
                white_list_user = White_List_Users(username=i, instagram_account=active_ig_account,
                                                   assistant=white_list_assistant)
                white_list_user.save()
            else:
                pass
        ...
        #     active_assistant.save()
            ...
            for i in user_followings:
                ...
                if len(check_ig_user) == 0:
                    ...
                    user_object = IG_Users(username=username, pk_number=pk, full_name=full_name, is_private=is_private,
                ...
                white_list_user = White_List_Users.objects.filter(instagram_account=active_ig_account,
                                                                  username=i.get('username'))
                unfollow_action = Unfollow_Actions.objects.filter(ig_user__username=i.get('username'),
                                                                  instagram_account=active_ig_account,
                                                                  assistant=active_assistant)
                if len(white_list_user) == 0 and len(unfollow_action) == 0:
                    unfollow_action = Unfollow_Actions(instagram_account=active_ig_account, ig_user=ig_user, status=0,
                                                       assistant=active_assistant,
                                                       update_time=datetime.now(timezone.utc))
                    unfollow_action.save()
                else:
...
def analyse_ig_account(username):
    try:
        ...
        ig_account_analysis = Instagram_Accounts_Analyse(instagram_account=instagram_account, media_count=media_count,
        ...
        for i in user_followers:
            ...
            try:
                ...
                Analyse = Analyse_FF(instagram_account=instagram_account, ig_user=user_object, is_follower=1,
                                     is_following=0, follower_update_time=datetime.now(timezone.utc))
                Analyse.save()
